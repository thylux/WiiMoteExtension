 ...
 */
 ...
 /*
  * No need to use int to pass a boolean. Switched from
  ...
  * to
  * void wiimote_write_buffer(byte *buffer, byte bdl, byte bdr, byte bdu, byte bdd,
  *		byte ba, byte bb, byte bx, byte by, byte blt, byte brt, byte bminus, byte bplus,
  *		byte bhome, byte lx, byte ly, byte rx, byte ry, byte bzl, byte bzr)
  *
  *
  *
  * Update includes to make compatible with Arduino 1.0
  *
  *
  *
  * Although referenced by the original comments that it wouldn't be needed to alter TWI_FREQ, had to redefine it to 400k, 
  * but now it doesn't need to be done in the original twi.h
  *
  */
